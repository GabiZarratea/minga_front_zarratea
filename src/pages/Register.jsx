...
export default function Register() {
  ...
  async function handleFormSubmit(event) {
    ...
    const formData = new FormData(); //metodo append agrega multiples campos al objeto formdata
    formData.append("email", email.current.value);
    formData.append("photo", photo.current.files[0]);
    formData.append("password", password.current.value);
    // console.log(photo.current.files[0], "photo.current.files[0]photo.current.files[0]");
          ...
          <form onSubmit={(e) => handleFormSubmit(e)} method="post" encType="multipart/form-data" className="w-full">
            <div className="w-full mt-8 mr-0 mb-0 ml-0 space-y-8 flex flex-col items-center">
              ...
              <p className="text-[#1F1F1F] text-sm not-italic font-medium leading-[normal] tracking-[0.7px]">
                ...
                <Anchor to={"/signin"} className="text-[color:var(--primary-two-design,#F97316)]">
                  Log in
                </Anchor>
              </p>
              <p className="text-[#1F1F1F] text-sm not-italic font-medium leading-[normal] tracking-[0.7px]">
